...
export function registerComponent (Vue, name, def) {
  Vue._fm_components_installed = Vue._fm_components_installed || {}
  const loaded = Vue._fm_components_installed[name]
  if (!loaded && def && name) {
    Vue._fm_components_installed[name] = true
...
export function registerDirective (Vue, name, def) {
  Vue._fm_vue_directives_ = Vue._fm_vue_directives_ || {}
  const loaded = Vue._fm_vue_directives_[name]
  if (!loaded && def && name) {
    Vue._fm_vue_directives_[name] = true
